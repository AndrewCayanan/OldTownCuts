...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($name) && !empty($specialty)) {
        $stmt = $conn->prepare("INSERT INTO barbers (name, specialty) VALUES (?, ?)");
        $stmt->bind_param("ss", $name, $specialty);

        if ($stmt->execute()) {
            header("Location: manage_barbers.php?success=Barber added successfully!");
            exit();
        } else {
...
<head>
    ...
    <title>Add Barber | Old Town Cuts</title>
